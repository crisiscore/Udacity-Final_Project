...
private const val CHANNEL_FACEBOOK = "Facebook"
private const val CHANNEL_TWITTER = "Twitter"
...
class RepresentativeViewHolder(val binding: ItemRepresentativeBinding): RecyclerView.ViewHolder(binding.root) {
    ...
    private fun showSocialLinks(channels: List<Channel>) {
        ...
        if (!facebookUrl.isNullOrBlank()) { visibleSocialLink(binding.imgFacebook, facebookUrl) }

        val twitterUrl = getTwitterUrl(channels)
        if (!twitterUrl.isNullOrBlank()) { visibleSocialLink(binding.imgTwitter, twitterUrl) }
    }

    private fun showWebLinks(urls: List<String>) {
        visibleSocialLink(binding.imgWebsite, urls.first())
    }

    private fun getFacebookUrl(channels: List<Channel>): String? {
        return channels.filter { channel -> channel.type == CHANNEL_FACEBOOK }
                .map { channel -> "https://www.facebook.com/${channel.id}" }
                .firstOrNull()
    }

    private fun getTwitterUrl(channels: List<Channel>): String? {
        return channels.filter { channel -> channel.type == CHANNEL_TWITTER }
                .map { channel -> "https://www.twitter.com/${channel.id}" }
                .firstOrNull()
    }

    private fun visibleSocialLink(view: ImageView, url: String) {
